---
title: "Quick Start"
description: "Get started with Qivryn's Agent mode to automatically implement code changes, fix bugs, and run commands using AI-powered tools that can modify your codebase based on natural language instructions"
---
...
<Info>
  ...
  model or provider doesn't support tools, or Qivryn doesn't yet support tools
...
Agent mode lives within the same interface as [Chat](/ide-extensions/chat/how-it-works) mode, so the same [input](/ide-extensions/chat/quick-start#how-to-start-a-conversation) is used to send messages and you can still use the same manual methods of providing context, such as [`@` context providers](/ide-extensions/chat/context-selection) or adding [highlighted code from the editor](/ide-extensions/chat/quick-start#how-to-include-code-context).
...
By default, Agent mode will ask permission when it wants to use a tool. Click `Qivryn` to allow Agent mode to proceed with the tool call or `Cancel` to reject it.
