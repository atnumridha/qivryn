...
<Warning>
  **This page documents deprecated functionality.** For creating slash commands and customizing Qivryn, please use:
...
Below are details for each property that can be set in `config.json`. The config schema code is found in [`extensions/vscode/config_schema.json`](https://github.com/atnumridha/qivryn/blob/main/extensions/vscode/config_schema.json).
...
```
"docs": [    {    "title": "Qivryn",    "startUrl": "https://docs.qivryn.ai/intro",    "faviconUrl": "https://docs.qivryn.ai/favicon.ico",  }]
```
  ...
  "slashCommands": [
    {
      ...
      "params": { "outputDir": "~/.qivryn/session-transcripts" }
    }
  ]
}
```
...
Write a custom slash command at your own HTTP endpoint. Set 'url' in the params object for the endpoint you have setup. The endpoint should return a sequence of string updates, which will be streamed to the Qivryn sidebar. See our basic [FastAPI example](https://github.com/atnumridha/qivryn/blob/74002369a5e435735b83278fb965e004ae38a97d/core/context/providers/context_provider_server.py#L34-L45) for reference.
...
Describe the issue you'd like to generate, and Qivryn will turn into a well-formatted title and body, then give you a link to the draft so you can submit. Make sure to set the URL of the repository you want to generate issues for.
  ...
  "slashCommands": [
    {
      ...
      "params": { "repositoryUrl": "https://github.com/atnumridha/qivryn" }
    }
  ]
}
```
