...
The `@Docs` context provider allows you to efficiently reference documentation directly within Qivryn.
...
By default, Qivryn crawls documentation sites using a specialized crawling service that provides the best experience for most users and documentation sites.
...
<Info>
  ...
  to `~/.qivryn/.utils`, and use it as the local crawler. **Note:** Chromium
...
- Qivryn does not automatically re-index your docs. Use `Click to re-index` to trigger a reindex for a specific source
...
You can also use the following IDE command to force a re-index of all docs: `Qivryn: Docs Force Re-Index`.
