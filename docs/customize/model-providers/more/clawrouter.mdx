---
title: "How to Configure ClawRouter with Qivryn"
...
Add multiple ClawRouter profiles to your config and switch via Qivryn's model picker:
...
Use the **model picker dropdown** in Qivryn's chat panel to switch between profiles. Each profile routes to different model tiers based on cost vs. quality trade-offs.

<Tip>
  **Quick switch via CLI:** In the Qivryn chat, type `/model` followed by the profile name (e.g., `/model ClawRouter Eco`).
</Tip>
...
You can configure ClawRouter for different Qivryn roles:
...
ClawRouter manages upstream provider API keys internally. You configure them in ClawRouter, not in Qivryn.
...
| Error | Qivryn's Default | ClawRouter's Handling |
|-------|-------------------|----------------------|
