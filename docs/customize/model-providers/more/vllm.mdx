---
title: "vLLM"
description: "Configure vLLM's high-performance inference library with Qivryn for chat, autocomplete, and embeddings, including setup instructions for Llama3.1, Qwen2.5-Coder, and Nomic Embed models"
---
...
Qivryn automatically handles vLLM's response format (which uses `results` instead of `data`).

[Click here](../../model-roles/reranking) to see a list of reranking model providers.

The Qivryn implementation uses [OpenAI](../top-level/openai) under the hood. [View the source](https://github.com/atnumridha/qivryn/blob/main/core/llm/llms/Vllm.ts)
