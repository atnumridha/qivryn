---
title: "Qivryn Autocomplete Setup and Configuration Guide"
description: "Step-by-step guide to setting up and configuring autocomplete in Qivryn, including Codestral, Ollama, and IDE settings."
...
## How to Set Up Autocomplete in Qivryn with Codestral (Recommended)
...
## How to Set Up Autocomplete in Qivryn with Ollama (Local Model)
...
Then, in the Qivryn panel, select this model as the default model for autocomplete.

## Autocomplete Configuration Options in Qivryn

### Customize Autocomplete User Settings in the Qivryn Extension
...
5. Check the output of the logs to find any potential errors: <kbd>cmd/ctrl</kbd> + <kbd>shift</kbd> + <kbd>P</kbd> -> "Toggle Developer Tools" -> "Console" tab in VS Code, ~/.qivryn/logs/core.log in JetBrains.
...
7. If you are still having issues, please file an issue on [GitHub](https://github.com/atnumridha/qivryn/issues).
...
4. Set the trigger condition (when) to `inlineSuggestionVisible && !editorReadonly`.
   This keeps multi-line completion from Qivryn, VS Code, and other extension snippets working. However, Tab will only fill in one line at a time. Any unnecessary code can be canceled with <kbd>Esc</kbd>.
   If you need to apply all the code, just press <kbd>Tab</kbd> multiple times.

### How to Turn Off Autocomplete in Qivryn (VS Code and JetBrains)

#### VS Code

Click the "Qivryn" button in the status panel at the bottom right of the screen. The checkmark will become a "cancel" symbol and you will no longer see completions. You can click again to turn it back on.

Alternatively, open VS Code settings, search for "Qivryn" and uncheck the box for "Enable Tab Autocomplete".
...
Open Settings -> Tools -> Qivryn and uncheck the box for "Enable Tab Autocomplete".

#### Feedback

If you're turning off autocomplete, we'd love to hear how we can improve! Please file an issue on [GitHub](https://github.com/atnumridha/qivryn/issues).
