---
title: "How to Create and Manage Rules in Qivryn"
...
1. Create a folder called `.qivryn/rules` at the top level of your workspace
2. Add a file called `pirates-rule.md` to this folder.
3. Write the following contents to `pirates-rule.md` and save.

```md title=".qivryn/rules/pirates-rule.md"
---
name: Pirate rule
---
...
For example, you can say "Create a rule for this", and a rule will be created for you in `.qivryn/rules` based on your conversation.
</Info>
...
globs: docs/**/*.{md,mdx}
alwaysApply: false
description: Standards for writing and maintaining Qivryn Docs
---

# Qivryn Docs Standards
...
You can create project-specific rules by adding a `.qivryn/rules` folder to the root of your project and adding new rule files.
...
```md title=".qivryn/rules/typescript.md"
---
...
**Check These**:
1. **File location**: Ensure rules are in `.qivryn/rules/` (not `.qivryn/rule/`)
2. **File format**: Rules should be `.md` files with proper YAML frontmatter

### How to Customize Chat System Message

Qivryn includes a simple default system message for [Agent mode](../../ide-extensions/agent/quick-start) and [Chat](../../ide-extensions/chat/quick-start) requests, to help the model provide reliable codeblock formats in its output.

This can be viewed in the rules section of the toolbar (see above), or in the source code [here](https://github.com/atnumridha/qivryn/blob/main/core/llm/constructMessages.ts#L4).
