---
title: "How to Configure Model Capabilities in Qivryn"
...
Qivryn needs to know what features your models support to provide the best experience. This guide explains how model capabilities work and how to configure them.

## What Are Model Capabilities?

Model capabilities tell Qivryn what features a model supports:
...
## How Qivryn Detects Model Capabilities

Qivryn uses a two-tier system for determining model capabilities:

### How Automatic Detection Works (Default)

Qivryn automatically detects capabilities based on your provider and model name. For example:
...
- [toolSupport.ts](https://github.com/atnumridha/qivryn/blob/main/core/llm/toolSupport.ts) - Tool capability detection logic
- [@qivryn/llm-info](https://www.npmjs.com/package/@qivryn/llm-info) - Image support detection

### How to Configure Capabilities Manually

You can add capabilities to models that Qivryn doesn't automatically detect in your `config.yaml`.

<Note>
  You cannot override autodetection - you can only add capabilities. Qivryn
...
2. **Using newer models** - Qivryn doesn't yet recognize a newly released model
...
Add tool support for a model that Qivryn doesn't recognize:
...
<Warning>
  An empty capabilities array does not disable autodetection. Qivryn will
...
3. **Keep Qivryn updated** - Newer versions improve autodetection

Remember: Setting capabilities only adds to autodetection. Qivryn will still use its built-in knowledge about your model in addition to your specified capabilities.

## Model Capability Support

This matrix shows which models support tool use and image input capabilities. Qivryn auto-detects these capabilities, but you can override them if needed.
