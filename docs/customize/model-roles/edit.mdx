...
In Qivryn, you can add `edit` to a model's roles to specify that it can be used for Edit requests. If no edit models are specified, the selected `chat` model is used.
...
You can customize the prompt template used for editing code by setting the `promptTemplates.edit` property in your model configuration. Qivryn uses [Handlebars syntax](https://handlebarsjs.com/guide/) for templating.
