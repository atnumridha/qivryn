...
sidebarTitle: "PostHog Analytics with Qivryn CLI"
---
...
<Card title="What You'll Build" icon="robot">
  A fully automated workflow that uses Qivryn CLI with the PostHog MCP to fetch analytics data, analyze user experience issues with AI, and automatically create GitHub
...
- [Qivryn CLI](https://docs.qivryn.ai/guides/cli) with **active credits** (required for API usage)
...
<Steps>
  ...
  <Step title="Set up Qivryn CLI Account & API Key">
        1. Visit [Qivryn Organizations](https://qivryn.ai/settings/organizations)
        2. Sign up or log in to your Qivryn account
        3. Navigate to your organization settings
        4. Click **"API Keys"** and then **"+ New API Key"** 
        5. Copy the API key immediately (you won't see it again!)
        6. Login to the CLI: `qivryn login`
  </Step>

  <Step title="Add Required Secrets to Qivryn CLI">
    Qivryn CLI will securely store your API keys as secrets that can be referenced in prompts.
  </Step>
</Steps>

<Tip>
  Qivryn CLI handles the complex API interactions - you just need to provide
  the right prompts!
</Tip>

## Step 1: Set Up Your Credentials

First, you'll need to gather your PostHog and GitHub API credentials and add them as secrets in Qivryn CLI.
...
<Tabs>
  <Tab title="PostHog API Credentials">
    ...
    3. Name it "Qivryn CLI Session Analysis"
    ...
    8. You'll also need your POSTHOG_AUTH_HEADER value, which is simply `Bearer YOUR_API_KEY`
      ...
      <Info>
  **Qivryn Secrets**: The `POSTHOG_AUTH_HEADER` secret should be stored in
  Qivryn's secure secrets storage. This keeps your API key safe and the MCP
...
<Tab title= "Configure API Access in Qivryn CLI">
...
<Tabs>
    ...
    **Perfect for:** Immediate results with optimized prompts and built-in debugging
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        ...
        ```bash
        qivryn --agent qivryn/posthog-continuous-ai-agent
        ```
      ...
      <Step title="Run the Analysis">
        ...
        ```bash
        qivryn "Give me my PostHog Session data and create GitHub issues based on the problems."
        ```
  ...
  <Steps>
    <Step title="Add PostHog MCP to Qivryn CLI">
    ...
    <Step title="Add PostHog + GitHub Analysis Rules">
      ...
      1. Pass `--rule bekah-hawrot-weigel/posthog-github-continuous-ai-rules` to `qivryn` OR
      2. Copy the rules to your local `.qivryn/rules` folder. See the [Rules Guide](/customize/deep-dives/rules#how-to-create-rules).
    </Step>
    <Step title="Create Your Custom Prompts">
      Use this prompt with Qivryn CLI to analyze PostHog data and create GitHub issues:
...
<Warning>
    ...
    **Protect Your API Keys:**
    ...
    code
    - Use Qivryn CLI's secure secret storage
...
Here are practical examples of what you can build with PostHog MCP and Qivryn CLI:
...
**Example Qivryn CLI prompts:**

```bash
# Get all feature flags and analyze them
qivryn "Use PostHog MCP to fetch all feature flags with feature-flag-get-all. Then analyze each flag to identify: 1) Flags that are 100% rolled out and could be removed, 2) Flags that haven't been updated in 90+ days, 3) Flags with complex targeting that might need simplification, 4) Experimental flags that should be cleaned up."

# Create cleanup issues for identified flags
qivryn "For each problematic feature flag identified, create a GitHub issue using gh CLI:
...
qivryn "Cross-reference feature flags with PostHog performance metrics to identify flags that may be impacting user experience or site performance. Create performance-focused GitHub issues for flags showing negative impact."
```
...
Consider enhancing your workflow with these advanced Qivryn CLI prompts:
...
- [Qivryn CLI Guide](https://docs.qivryn.ai/guides/cli)
- [Continuous AI Best Practices](https://blog.qivryn.ai/what-is-continuous-ai-a-developers-guide/)
