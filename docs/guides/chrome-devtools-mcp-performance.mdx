...
description: "Measure and optimize web performance with Chrome DevTools MCP, automated performance traces, and Core Web Vitals monitoring using Qivryn."
...
This guide shows you how to leverage these features through natural language with Qivryn CLI!
...
- [Qivryn CLI](https://docs.qivryn.ai/guides/cli)
- [Chrome DevTools MCP](https://qivryn.ai) configured
    ...
    **Perfect for:** Immediate results with optimized prompts and built-in performance analysis
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        ...
        ```bash
        qivryn --agent qivryn/chrome-dev-tools-agent
        ```

        This agent includes:
        - **Optimized prompts** for performance analysis and debugging
        - **Built-in rules** for consistent formatting and error handling
        - **[Chrome DevTools MCP](https://qivryn.ai/google/chrome-devtools-mcp)** for reliable browser automation
      </Step>
...
Now you can use natural language prompts to analyze web performance. The Qivryn agent automatically calls the appropriate Chrome DevTools MCP tools.
...
This workflow automatically validates web performance on pull requests using the Qivryn CLI in headless mode. It records performance traces, extracts Core Web Vitals, and posts a summary report as a PR comment.
...
| Issue | Quick Fix Command (in qivryn TUI) |
| :---- | :---------------------------- |
...
<Card title="Continuous AI" icon="rocket">
  Your performance workflow now operates at **[Level 2 Continuous AI](https://blog.qivryn.ai/what-is-continuous-ai-a-developers-guide/)** - AI handles routine performance analysis and debugging with human oversight through review and approval of changes.
</Card>
