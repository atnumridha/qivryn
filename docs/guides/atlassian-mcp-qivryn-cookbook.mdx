---
title: "Jira Issues and Confluence Pages with Atlassian MCP and Qivryn"
description: "Use Qivryn and the Atlassian Rovo MCP to search, summarize, and manage Jira issues, Confluence pages, and Compass components with natural language prompts."
sidebarTitle: "Atlassian Workflows with Qivryn"
---
...
<Card title="What You'll Build" icon="atlassian">
  An Atlassian workflow assistant that uses Qivryn with the Atlassian Rovo MCP to:
...
<Warning>
  To use agents in headless mode, you need a [Qivryn API key](https://qivryn.ai/settings/api-keys).
...
|-------|----------|-----------|------|
| **Jira Agent** | Issue Management | Search issues, create stories, sprint planning, status updates, bulk operations | [View Agent](https://qivryn.ai/qivryn/atlassian-continuous-ai-jira-agent) |
| **Confluence Agent** | Documentation | Search pages, summarize docs, create/update pages, manage spaces | [View Agent](https://qivryn.ai/qivryn/atlassian-continuous-ai-confluence-agent) |

<Note>
  **Cross-product workflows**: Both agents can work with Jira, Confluence, and Compass. Choose based on your primary focus area, or create your own agent using the [Atlassian MCP](https://qivryn.ai/atlassian/atlassian-mcp). The Atlassian MCP can work with Jira, Compass, or Confluence.
</Note>
  ...
  <Tab title="📋 Jira Agent (Recommended for Issue Management)">
    <Steps>
      ...
      <Step title="Run Your First Command">
        ...
        ```bash
        qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Show me my open issues assigned to me"
        ```
    ...
    <Info>
      **Pro tip**: Create a shell alias for the Jira agent:
      ```bash
      alias jira-ai='qivryn --agent qivryn/atlassian-continuous-ai-jira-agent'
      ```
  ...
  <Tab title="📄 Confluence Agent (Recommended for Documentation)">
    <Steps>
      ...
      <Step title="Run Your First Command">
        From anywhere in your terminal:
        ```bash
        qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent "Summarize the Q2 planning page"
        ```
        The agent will connect to your Atlassian site and return results.
      </Step>
    </Steps>
    ...
    <Info>
      **Pro tip**: Create a shell alias for the Confluence agent:
      ```bash
      alias confluence-ai='qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent'
      ```
...
<Accordion title="Agent Requirements">
    To use Atlassian MCP with Qivryn CLI, you need either:
      - **Qivryn CLI Pro Plan** with the models add-on, OR
...
<Note>
  **Using the Jira Agent**: All examples below use the [Jira Agent](https://qivryn.ai/qivryn/atlassian-continuous-ai-jira-agent) which is optimized for issue management.
</Note>
...
<Info>
  **Running in headless mode**: Add `-p` flag before your prompt and `--auto` flag at the end:
  ```bash
  qivryn --agent qivryn/atlassian-continuous-ai-jira-agent -p "your prompt" --auto
  ```
...
<AccordionGroup>
  <Accordion title="Find Open Bugs" icon="bug">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Find all open bugs in Project Alpha"
    ```
  </Accordion>
  ...
  <Accordion title="My Assigned Issues" icon="user">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Show me all issues assigned to me that are in progress"
    ```
  </Accordion>
  ...
  <Accordion title="Sprint Planning" icon="calendar">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "List all issues in the current sprint. Group by assignee and priority. Summarize the workload distribution and flag any overloaded team members."
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Create Story from Description" icon="plus">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Create a story titled 'Redesign onboarding flow' in Project Alpha. Description: We need to simplify the user registration process by reducing steps from 5 to 3. Target completion: Q3."
    ```
  </Accordion>
  ...
  <Accordion title="Bulk Issue Creation" icon="list">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Create five Jira issues from these meeting notes: 1) Fix login timeout bug 2) Update API documentation 3) Implement dark mode toggle 4) Review security audit findings 5) Optimize database queries"
    ```
  </Accordion>
  ...
  <Accordion title="Update Issue Status" icon="arrows-rotate">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Move PROJ-123 to In Progress and add a comment: Starting work on this today. ETA: end of week."
    ```
  </Accordion>
</AccordionGroup>
...
<Note>
  **Using the Confluence Agent**: All examples below use the [Confluence Agent](https://qivryn.ai/qivryn/atlassian-continuous-ai-confluence-agent) which is optimized for documentation management.
</Note>

Access, search, and manage your team's documentation directly from Qivryn.

<Info>
  **Running in headless mode**: Add `-p` flag before your prompt and `--auto` flag at the end:
  ```bash
  qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent -p "your prompt" --auto
  ```
  **Important**: Complete browser OAuth authentication first before using headless mode.
</Info>
...
<AccordionGroup>
  <Accordion title="Summarize Documentation" icon="book">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent "Summarize the Q2 planning page from the Engineering space"
    ```
  </Accordion>
  ...
  <Accordion title="Find Documentation" icon="magnifying-glass">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent "Find all pages about API authentication in our developer docs"
    ```
  </Accordion>
  ...
  <Accordion title="List Available Spaces" icon="folder">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent "What Confluence spaces do I have access to?"
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Create Documentation Page" icon="file-plus">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent "Create a page titled 'Team Goals Q3' in the Engineering space. Include sections for: Objectives, Key Results, and Timeline."
    ```
  </Accordion>
  ...
  <Accordion title="Update Existing Page" icon="pen-to-square">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent "Update the 'Onboarding Guide' page to add a new section about our code review process"
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Query Dependencies" icon="diagram-project">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "What services depend on the api-gateway component?"
    ```
  </Accordion>
  ...
  <Accordion title="Create Service Component" icon="server">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Create a service component for the current repository. Use the package.json to infer details."
    ```
  </Accordion>
  ...
  <Accordion title="Bulk Import Components" icon="upload">
    ...
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Import Compass components and custom fields from this CSV: [paste CSV data]"
    ```

    Or reference a file:
    ```bash
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Import Compass components from services.csv in the current directory"
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Link Jira to Confluence" icon="link">
    ...
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Link Jira tickets PROJ-123, PROJ-456, and PROJ-789 to the 'Sprint 23 Release Plan' Confluence page"
    ```
  </Accordion>
  ...
  <Accordion title="Service Documentation Lookup" icon="search">
    ...
    qivryn --agent qivryn/atlassian-continuous-ai-confluence-agent "Fetch the Confluence documentation page linked to the user-authentication Compass component"
    ```
  </Accordion>
  ...
  <Accordion title="Release Notes Generation" icon="file-lines">
    ...
    qivryn --agent qivryn/atlassian-continuous-ai-jira-agent "Generate release notes for all Jira issues completed in Sprint 23. Create a Confluence page in the Release Notes space with: New features (grouped by epic), Bug fixes, Known issues, Deployment instructions"
    ```
  </Accordion>
</AccordionGroup>
...
<Card title="Continuous AI" icon="rocket">
  ...
  AI](https://blog.qivryn.ai/what-is-continuous-ai-a-developers-guide/)** -
...
<CardGroup cols={2}>
  ...
  <Card title="MCP in Qivryn" icon="book" href="/customize/deep-dives/mcp">
    How MCP works with Qivryn agents
  </Card>
  <Card title="Qivryn Documentation" icon="book" href="https://docs.qivryn.ai">
    Qivryn documentation and guides
  </Card>
