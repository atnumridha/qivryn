...
description: "Learn how to audit Row Level Security in your Supabase database, identify vulnerabilities, and automatically generate fixes using Qivryn CLI and Supabase MCP."
...
<Card title="What You'll Build" icon="shield-check">
  A security audit workflow that uses Qivryn CLI with Supabase MCP to identify RLS vulnerabilities, generate secure policies, fix permission issues, and ensure your database follows security best practices.
</Card>
...
<Tip>
  Qivryn CLI can analyze your database schema and generate complex SQL queries - you just need to describe what you want in plain language!
</Tip>
...
<Tabs>
    ...
    **Perfect for:** Immediate database analysis with AI-powered query optimization and schema insights
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        ...
        ```bash
        qivryn --agent qivryn/supabase-agent
        ```
      ...
      <Step title="Run Database Analysis">
        Navigate to your project directory and enter this prompt in the Qivryn CLI TUI:
  ...
  <Tab title="🛠️ Manual Setup">
    <Steps>
      <Step title="Add Supabase MCP to Qivryn CLI">
      ...
      <Step title="Create Custom Database Analysis Prompts">
        Use this prompt template with Qivryn CLI to analyze your database:
...
Use Qivryn CLI to perform intelligent database analysis. Enter these prompts in the Qivryn CLI TUI:
...
Create and apply database migrations based on AI recommendations. Enter this prompt in the Qivryn CLI TUI:
...
Automate database health checks with Qivryn CLI and GitHub Actions:
    ...
    steps:
      ...
      - name: Analyze Database Security
        ...
        run: |
          ...
          # Use Qivryn CLI to audit RLS and generate fixes
          qivryn -p "Using Supabase MCP, perform a comprehensive RLS security audit:
...
<Card title="Continuous AI Database Management" icon="shield-check">
  Your system now operates at **[Level 2 Continuous AI](https://blog.qivryn.ai/what-is-continuous-ai-a-developers-guide/)** - AI handles routine database analysis and optimization with human oversight for migration approval.
</Card>
