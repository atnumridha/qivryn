---
title: "Building Data Pipelines with dlt MCP and Qivryn"
...
sidebarTitle: "dlt Data Pipelines with Qivryn"
---
...
<Card title="What You'll Build" icon="database">
  An AI-powered data pipeline development system that uses Qivryn's AI agent with dlt
...
<Card title="🚀 Fastest Path to Success" icon="zap">
  Skip the manual setup and use our pre-built [dlt Agent](https://qivryn.ai/qivryn/dlt-agent) that includes
...
<Steps>
  <Step title="Add dlt MCP to Qivryn CLI">
    Add the dlt MCP to your configuration:

    <Tip>
      **Installation methods:**
      1. **Quick CLI install**: `qivryn --mcp dlthub/dlt-mcp`
      2. **Manual configuration**: Add the MCP to your `~/.qivryn/config.json` under the `mcpServers` section

      Once installed, dlt MCP tools become available to your Qivryn agent for all prompts.
    </Tip>
...
<Accordion title="Agent Requirements">
    To use the pre-built [dlt Agent](https://qivryn.ai/qivryn/dlt-agent), you need either:
...
<Card title="Understanding the Difference" icon="info-circle">
  ...
  **[dlt+ MCP](https://qivryn.ai/dlthub/dlt-plus-mcp)** extends these capabilities with cloud-based features for production deployments:
  ...
  For local development and getting started, **[dlt MCP](https://qivryn.ai/dlthub/dlt-mcp)** is the right choice. Consider **[dlt+ MCP](https://qivryn.ai/dlthub/dlt-plus-mcp)** when you need production deployment features and team collaboration.
</Card>
...
Now you can use natural language prompts to develop and debug your dlt pipelines. The Qivryn agent automatically calls the appropriate dlt MCP tools.
...
<Info>
  **Where to run these workflows:**
  ...
  - **CLI (headless mode)**: Use `qivryn -p "your prompt"` for headless commands
  ...
  **Test in Plan Mode First**: Before running pipeline operations that might make
  ...
  To run any of the example prompts below in headless mode, use `qivryn -p "prompt"`
</Info>
...
## Continuous Data Pipelines with GitHub Actions
  ...
  This example demonstrates a **Continuous AI workflow** where data pipeline validation runs automatically in your CI/CD pipeline in headless mode using the [dlt Assistant agent](https://qivryn.ai/dlthub/dlt-assistant). Consider customizing this agent to add your organization's specific validation rules.
...
<Info>
      The workflow uses the pre-built [dlt Agent](https://qivryn.ai/qivryn/dlt-agent) with `--agent qivryn/dlt-agent`. This agent comes pre-configured with the dlt MCP and optimized rules for pipeline operations. You can customize the validation rules and prompts for your specific pipeline requirements.
</Info>

### Create Workflow File

This workflow automatically validates your dlt data pipelines on pull requests using the Qivryn CLI in [headless mode](/cli/headless-mode). It inspects pipeline schemas, checks for errors, and posts a summary report as a PR comment. The workflow can also be triggered manually via `workflow_dispatch`.
  ...
  validate-pipeline:
    ...
    steps:
      ...
      - name: Comment Pipeline Report on PR
        ...
        run: |
          REPORT=$(qivryn --agent qivryn/dlt-agent \
...
Implement automated pipeline quality checks using Qivryn's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="newspaper"
    href="https://dlthub.com/blog/deep-dive-assistants-mcp-qivryn"
  >
    Learn about AI agents, MCP, and Qivryn integration
  </Card>
  <Card
    title="MCP Concepts"
    icon="puzzle-piece"
