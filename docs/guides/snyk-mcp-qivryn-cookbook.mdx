---
title: "Automated Security Scanning with Snyk MCP and Qivryn"
...
sidebarTitle: "Snyk Security Scanning with Qivryn"
---
...
<Card title="What You'll Build" icon="shield-halved">
  An automated security scanning system that uses Qivryn's AI agent with Snyk
...
<iframe
  ...
  title="Snyk MCP Qivryn Cookbook Demo"
  frameborder="0"
...
<Note>
  **Important**: The Snyk MCP requires the Snyk CLI to be authenticated locally. Run `snyk auth` to authenticate before using the Qivryn agent with Snyk MCP.
</Note>
 <Warning>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        qivryn --agent qivryn/snyk-continuous-ai-agent
        ```
      ...
      <Step title="Run Your First Security Scan">
        ...
        ```bash
         # Headless mode
        qivryn -p "Run a complete security scan on this project including code vulnerabilities, dependencies, and any IaC files. Summarize findings by severity." --auto
        ```
...
<Steps>
  <Step title="Add Snyk MCP to Qivryn CLI">
    ...
    <Tip>
      **Installation methods:**
      1. **Quick CLI install**: `qivryn --mcp snyk/snyk-mcp`
      2. **Manual configuration**: Add the MCP to your `~/.qivryn/config.json` under the `mcpServers` section

      Once installed, Snyk MCP tools become available to your Qivryn agent for all prompts.
    </Tip>

    <Info>
      The MCP will request authentication and folder trust permissions when first used.
      This is handled automatically by the Qivryn agent.
    </Info>
  ...
  <Step title="Run Your First Security Scan">
    ...
    ```bash
    # TUI mode
    qivryn "Run a complete security scan on this project including code vulnerabilities, dependencies, and any IaC files. Summarize findings by severity."
    ```
  </Step>
</Steps>
...
Now you can use natural language prompts to run comprehensive security scans. The Qivryn agent automatically calls the appropriate Snyk MCP tools.


<Info>
  **Where to run these workflows:**
  - **IDE Extensions**: Use Qivryn in VS Code, JetBrains, or other supported IDEs
  - **Terminal (TUI mode)**: Run `qivryn` to enter interactive mode, then type your prompts
  - **CLI (headless mode)**: Use `qivryn -p "your prompt" --auto` for headless commands
...
```bash
qivryn -p "Run a Snyk Code scan on this repo with severity threshold medium. Summarize issues with file:line. Propose minimal diffs for the top 3 and rerun to verify." --auto
```
...
**Headless Mode Prompt:**
```bash
qivryn -p "Run Snyk Open Source on this repo (include dev deps). Summarize vulnerable paths and propose a minimal-risk upgrade plan. Re-test after the plan (dry run)." --auto
```
...
**Headless Mode Prompt:**
```bash
qivryn -p "Scan ./infra with Snyk IaC. Report high/critical misconfigs with exact files/lines. Provide code changes and re-scan to confirm." --auto
```
...
**Headless Mode Prompt:**
```bash
qivryn -p "Scan image my-api:latest. Exclude base image vulns. Print dependency tree. Recommend a safer base image or upgrades. Re-test after the change (dry run)." --auto
```
...
**Headless Mode Prompt:**
```bash
qivryn -p "Scan only files changed since origin/main with Snyk Code. Block if new high issues would be introduced. Show deltas." --auto
```
...
**Headless Mode Prompt:**
```bash
qivryn -p "Open Snyk Learn lessons related to the top CWE(s) from this scan." --auto
```
...
- `QIVRYN_API_KEY`: Your Qivryn API key from [qivryn.ai/settings/api-keys](https://qivryn.ai/settings/api-keys)
    ...
    steps:
      ...
      - name: Validate Secrets
        run: |
          ...
          if [ -z "${{ secrets.QIVRYN_API_KEY }}" ]; then
            echo "⚠️ Warning: QIVRYN_API_KEY not set - AI mitigation suggestions will be skipped"
          fi
      ...
      - name: Generate AI Mitigation Suggestions
        ...
        run: |
          ...
          # Create a summary of findings for Qivryn CLI
          ...
          if [ -n "$FINDINGS_SUMMARY" ]; then
            ...
            # Use Qivryn CLI to generate mitigation suggestions
            ...
            qivryn --agent qivryn/snyk-continuous-ai-agent -p "$PROMPT" --auto > mitigation-suggestions.md || {
      ...
      - name: Post Mitigation Summary to PR
        ...
        run: |
          ...
          **Scan Details:**
          ...
          - 🤖 Generated with Qivryn CLI + Snyk
...
<Info>
  **About SNYK_TOKEN**: The workflow uses the SNYK_TOKEN in two ways:
  ...
  2. **Qivryn CLI access** - Available as an environment variable when Qivryn generates AI mitigation suggestions

  The `qivryn` agent automatically uses the SNYK_TOKEN when needed for Snyk MCP operations.
</Info>

<Info>
  This workflow demonstrates several advanced features:
  - **Changed Files Detection**: Only scans files modified in the PR
  - **AI Mitigation**: Uses Qivryn CLI to generate actionable mitigation steps
...
Implement automated security policies using Qivryn's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
...
<CardGroup cols={2}>
  ...
  <Card title="Qivryn Documentation" icon="book" href="https://docs.qivryn.ai">
    Explore Qivryn documentation and guides
  </Card>
  <Card
