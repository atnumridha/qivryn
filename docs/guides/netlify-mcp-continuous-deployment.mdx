...
description: "Optimize web performance with A/B testing, automated monitoring, and data-driven improvements using Netlify MCP and Qivryn."
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        ...
        ```bash
        qivryn --agent qivryn/netlify-continuous-ai-agent
        ```
      ...
      <Step title="Run Performance Analysis">
        ...
        ```bash
        qivryn "Analyze my Netlify site's performance and optimize it for better Core Web Vitals."
        ```
...
<Step title="Configure Netlify MCP">
  ...
  <Step title="Verify Setup">
    Test the connection with qivryn CLI:
 ...
 <Info>
    You're all set! Now you can use qivryn CLI to interact with Netlify using natural language prompts. Check out the examples below to get started.
  </Info>
  </Tab>
</Tabs>
...
Establish your current performance baseline using qivryn CLI:

```bash
# Start qivryn in TUI mode
qivryn
...
<Tip>
  **Discover more optimization prompts!** The Netlify community has documented dozens of build optimization strategies in their [Support Guide: How can I optimize my Netlify build time](https://answers.netlify.com/t/support-guide-how-can-i-optimize-my-netlify-build-time/3907). 
  ...
  Use this guide as inspiration for qivryn CLI prompts not covered in this cookbook, such as:
  ...
  The community guide contains real-world scenarios that you can turn into AI-assisted solutions - just describe what you want to achieve and let qivryn CLI handle the implementation!
</Tip>
...
jobs:
  performance:
    ...
    steps:
      ...
      - name: Authenticate Qivryn CLI
        env:
          QIVRYN_API_KEY: ${{ secrets.QIVRYN_API_KEY }}
        run: |
          qivryn auth login --api-key "$QIVRYN_API_KEY"
          echo "✅ Qivryn CLI authenticated"
      ...
      - name: Deploy and Test Performance
        ...
        run: |
          ...
          qivryn -p "Deploy PR preview and run Lighthouse.
...
```bash
# Try these advanced features in qivryn TUI mode:
...
- Install the Netlify MCP in your Qivryn configuration
...
- [Qivryn Performance Guides](https://docs.qivryn.ai/guides)
