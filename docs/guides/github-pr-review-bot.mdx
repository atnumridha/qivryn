---
title: "Code Review Bot with Qivryn and GitHub Actions"
description: "Set up automated, context-aware pull request reviews using Qivryn CLI in GitHub Actions - privacy-first with custom rules"
...
<CardGroup cols={3}>
  <Card title="Privacy-First" icon="shield">
    All logs and processing happen in your runner: Qivryn CLI runs in GitHub Actions → code to your LLM provider (OpenAI, Anthropic, etc.). No hosted Qivryn service reads your code.

  </Card>

  <Card title="Customizable" icon="sliders">
    Define team-specific rules in `.qivryn/rules/` that automatically apply to every pull request.

  </Card>

  <Card title="Context Awareness" icon="robot">
    Leverage Qivryn's AI agent for intelligent, context-aware reviews with full control over your configuration.
...
- A Qivryn API key from [qivryn.ai/settings/api-keys](https://qivryn.ai/settings/api-keys)
- Qivryn assistant configured for code reviews (or use our recommended default)

<Info>
  **Want to customize the review bot?**
  
  You can remix the default review bot configuration at [qivryn.ai/qivryn/review-bot](https://qivryn.ai/qivryn/review-bot) to create your own personalized version with custom prompts, rules, and behaviors.
</Info>
...
jobs:
  review:
    ...
    steps:
      ...
      - name: Run Qivryn Review
        env:
          QIVRYN_API_KEY: ${{ secrets.QIVRYN_API_KEY }}
          GH_TOKEN: ${{ steps.app-token.outputs.token || github.token }}
        run: |
          # Check if custom rules exist
          if [ -d ".qivryn/rules" ]; then
            echo "📋 Found custom rules in .qivryn/rules/"
            RULES_CONTEXT="Apply the custom rules found in .qivryn/rules/ directory."
          else
          ...
          # Run Qivryn CLI in headless mode
          qivryn --config qivryn/review-bot \
             -p "$PROMPT" \
      ...
      - name: Post Review Comment
        ...
        run: |
          ...
          ---
          *Powered by [Qivryn](https://qivryn.ai) • Need a focused review? Comment `@review-bot check for [specific concern]`*
          EOF
...
Define your team's standards in `.qivryn/rules/`:

<Tabs>
  <Tab title="Security Rule">
    
 Create `.qivryn/rules/security.md`:
...
```
  ...
  <Tab title="TypeScript Standards">
    Create `.qivryn/rules/typescript.md`:
...
Create `.qivryn/rules/testing.md`:
...
3. **Load Custom Rules** - Reads your team's rules from `.qivryn/rules/`
4. **Get Pull Request Diff** - Fetches the diff and list of changed files
5. **Qivryn CLI Analyzes Code** - AI agent reviews the code with your rules
...
<Tabs>
  <Tab title="Use Your Own Qivryn Config">
    
  By default, the workflow uses the `qivryn/review-bot` config optimized for code reviews. Replace `qivryn/review-bot` with your own config:

  ```yaml
  - name: Run Qivryn Review
    env:
      QIVRYN_API_KEY: ${{ secrets.QIVRYN_API_KEY }}
      QIVRYN_ORG: your-org-name      # Add your org
      QIVRYN_CONFIG: username/config-name  # Add your config
    run: |
  ...
  Store `QIVRYN_ORG` and `QIVRYN_CONFIG` as repository variables for easy updates.
</Tab>
...
## Troubleshooting
<AccordionGroup>
<Accordion title="Qivryn CLI not found">
- The workflow installs the CLI automatically, but ensure Node.js 20+ is available
- Check the "Install Qivryn CLI" step logs for errors
</Accordion>
<Accordion title="Authentication failed">
- Verify your `QIVRYN_API_KEY` is valid
- Check that GitHub token has required permissions
</Accordion>
<Accordion title="No review generated">
- Verify your Qivryn config is accessible
- Check Qivryn CLI logs in the workflow run
- Try running locally: `qivryn -p "Test prompt" --auto`
</Accordion>
...
>
> ### Recommendations
> 1. Move `secretKey` to environment variables (see `.qivryn/rules/security.md`)
...
>
> ---
> *Powered by [Qivryn](https://qivryn.ai) • Need a focused review? Comment `@review-bot check for security`*
...
5. **Create team config** - Set up a shared Qivryn config for consistent reviews
...
<CardGroup cols={2}>
  ...
  <Card title="Qivryn CLI Guide" icon="terminal" href="/guides/cli">
    Learn more about Qivryn CLI capabilities
  </Card>
