...
<Card title="What You'll Build" icon="bug">
  An automated error monitoring system that uses Qivryn CLI with Sentry MCP to analyze production errors, identify root causes with AI, and create detailed GitHub issues with suggested fixes.
</Card>
...
<Tip>
  Qivryn CLI handles complex error analysis and API interactions - you just need to provide the right prompts!
</Tip>
...
<Tabs>
  <Tab title="Configure Sentry MCP">
    ...
    The Sentry MCP supports multiple configuration methods. For Qivryn CLI, OAuth is recommended:
  ...
  <Tab title="Sentry API Credentials">
    ...
    3. Name it "Qivryn CLI Error Analysis"
...
<Steps>
  <Step title="Add Sentry MCP to Qivryn CLI">
  ...
  <Step title="Create Custom Error Analysis Prompts">
    Use this prompt template with Qivryn CLI to analyze Sentry errors:
...
Use Qivryn CLI to perform intelligent error analysis. Enter these prompts in the Qivryn CLI TUI:
...
Create actionable GitHub issues from Sentry errors. Enter this prompt in the Qivryn CLI TUI:
...
Automate error monitoring with the [Sentry Release GitHub Action](https://docs.sentry.io/product/releases/setup/release-automation/github-actions/) and Qivryn CLI to create comprehensive, AI-powered issue descriptions:

<Tip>
  **Why Combine Sentry Releases with Qivryn CLI?**
  - **Release Tracking**: Associate errors with specific deployments
  - **AI-Powered Analysis**: Qivryn CLI generates detailed issue descriptions with root cause analysis
    ...
    steps:
      ...
      - name: Analyze Sentry Errors and Create Issues
        ...
        run: |
          ...
          # Use Qivryn CLI to analyze errors and generate comprehensive issue descriptions
          qivryn -p "Using Sentry MCP, analyze errors from the past 6 hours for project $SENTRY_PROJECT:
...
<Info>
  **Workflow Best Practices**:
  ...
  - Use Qivryn CLI to generate comprehensive, AI-powered issue descriptions
...
<Card title="Continuous AI Error Monitoring" icon="shield-check">
  Your system now operates at **[Level 2 Continuous AI](https://blog.qivryn.ai/what-is-continuous-ai-a-developers-guide/)** - AI handles routine error analysis with human oversight through GitHub issue review and resolution.
</Card>

## Advanced Error Analysis Prompts

Enhance your workflow with these advanced Qivryn CLI prompts:
...
| OAuth prompt not appearing | Check that Qivryn CLI has proper MCP configuration |
