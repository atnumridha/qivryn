---
title: "GitHub Issues and PRs with GitHub MCP and Qivryn"
description: "Use Qivryn and the GitHub MCP to list, summarize, and act on open issues and recently merged pull requests with natural language prompts."
sidebarTitle: "GitHub Issues with Qivryn"
---
...
<Card title="What You'll Build" icon="github">
  A GitHub workflow assistant that uses Qivryn with the GitHub MCP to:
...
<Card title="Fastest Path to Success" icon="rocket">
  Use the GitHub MCP with Qivryn CLI for quick setup.
</Card>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        Run:
        ```bash
        qivryn --agent qivryn/github-project-manager-agent
        ```
    ...
    <Info>
      You can also attach an MCP to a one-off session: `qivryn --mcp anthropic/github-mcp`.
    </Info>
  </Tab>
...
<Accordion title="Agent Requirements">
    To use GitHub MCP with Qivryn CLI, you need either:
...
<Info>
  **Where to run these workflows:**
  ...
  - **CLI (headless mode)**: Use `qivryn -p "your prompt" --auto` for automation
...
- `QIVRYN_API_KEY`: From [qivryn.ai/settings/api-keys](https://qivryn.ai/settings/api-keys)
  ...
  report:
    ...
    steps:
      ...
      - name: Post PR Context Comment
        ...
        run: |
          REPORT=$(        qivryn --agent qivryn/github-project-manager-agent \
...
<Info>
  Use `qivryn/github-project-manager-agent` consistently throughout your automation as the recommended agent configuration.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub Project Manager Agent" icon="robot" href="https://docs.qivryn.ai/guides/github-mcp-qivryn-cookbook">
