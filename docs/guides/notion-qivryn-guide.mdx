---
title: "Developer & Team Workflows with Notion + Qivryn CLI"
description: "Use Qivryn CLI with Notion to generate docs, manage tasks, and automate project workflows – all through natural-language prompts."
sidebarTitle: "Notion with Qivryn"
---
...
- Use natural language to connect to the Notion API directly with Qivryn CLI for powerful automation
...
- [Qivryn CLI](https://docs.qivryn.ai/cli/quickstart) installed
- A [Notion workspace](https://notion.so) with Editor (or higher) access
- Node.js 18+ installed locally
- [Qivryn account](https://qivryn.ai)

<Warning>

  **Agent usage requires credits** – create a Qivryn API key at
  [qivryn.ai/settings/api-keys](https://qivryn.ai/settings/api-keys)
...
<Steps>
  ...
  <Step title="Create Notion Integration & Get API Key">
    ...
    2. Click **+ New integration** → give it a name (e.g. "Qivryn Integration")
  ...
  <Step title="Add Workspace Keys to Terminal Session">
    Depending on what you want `qivryn` to accomplish, you'll need to add your Notion workspace keys to the terminal session. The workspace key is your Notion database ID. Run the following command:
...
## Running Qivryn CLI with Notion API

<Card title="🚀 Choose Your Interface" icon="zap">

  Qivryn CLI offers two powerful modes for Notion automation:
...
<Accordion title="API Connection Notes">
  ...
  - Environment variable `NOTION_API_KEY` must be set before running Qivryn CLI
  - Qivryn automatically uses the API key to authenticate with Notion
  - No need for manual curl commands - just reference "the API key stored in this session"
  - For complex workflows, Qivryn maintains the API connection throughout
...
Working example that demonstrates the power of Qivryn with Notion API:
...
<CardGroup cols={1}>
  <Card title="API Documentation" icon="code">
    ...
    **Headless Mode:**
    ```bash
    qivryn -p --auto "Fetch my Notion databases using the API key and secrets stored in this session. Generate API docs
...
<Accordion title="Common Issues and Solutions">
  ...
  **Connection Issues:**
  - Verify Qivryn CLI has internet access
