---
title: "Automating Documentation Updates with Qivryn CLI"
sidebarTitle: "Automating Documentation Updates with Qivryn CLI"
description: "Learn how to create automated documentation generation workflows using Qivryn CLI. Set up AI agents to analyze code changes and generate or update documentation automatically in GitHub workflows or local development."
---
...
This guide demonstrates how to create automated documentation generation based on code updates in a git branch using the Qivryn CLI, either as part of your local workflow or as part of a GitHub workflow.

This process utilizes the **Qivryn CLI** (`qivryn`) in **headless mode** to analyze changes and generate the necessary documentation, and commit and push the changes. The goal is to keep the workflow as simple as possible by using straightforward shell commands, Qivryn CLI prompts, and basic git operations.

## Why Use Qivryn CLI for Documentation?
...
<CardGroup cols={1}>
  <Card title="Install Qivryn CLI" icon="terminal">
    Qivryn CLI requires Node.js 20 or higher.
    
    <CLIInstall />
  </Card>

  <Card title="Qivryn API Key" icon="key">
    Get your API key from [qivryn.ai/settings/api-keys](https://qivryn.ai/settings/api-keys) and set:
    ```bash
    export QIVRYN_API_KEY=your_key_here
    ```
    <Info>
    You can use the Qivryn CLI in headless mode without interactive login by setting the `QIVRYN_API_KEY` environment variable.
    </Info>
  </Card>
...
<Steps>
  <Step title="Environment Setup">
    Validate environment, install Qivryn CLI, and set up authentication with API keys.
  </Step>
  ...
  <Step title="AI Documentation Generation">
    Use Qivryn CLI with custom rules to analyze changes and generate or update documentation files.
    <Tip>
...
- **qivryn_config:** The Qivryn agent configuration to use
  - Consider setting a default value if you have a default config your'd like to use
- **qivryn_org:** The Qivryn org to use
...
jobs:
  write-docs:
    ...
    env:
      ...
      QIVRYN_ORG: ${{ github.event.inputs.qivryn_org }}
      QIVRYN_CONFIG: ${{ github.event.inputs.qivryn_config }}
    ...
    - name: Install Qivryn CLI
      run: |
        echo "Installing Qivryn CLI..."
        npm i -g @qivryn/cli

    - name: Verify Qivryn CLI installation
      run: |
        echo "Checking Qivryn CLI version..."
        qivryn --version || exit 1
    ...
    - name: Generate documentation with Qivryn CLI
      run: |
        echo "Running Qivryn agent to generate documentation..."
        qivryn --config <qivryn-user>/<agent-config> \
           --auto \
           --allow Write \
           -p \
           --prompt ./context.txt \
...
<Warning>
Make sure to set your `QIVRYN_API_KEY` environment variable before running local scripts to enable headless mode.
</Warning>

When using the Qivryn CLI on your local machine, you can build workflows in various ways, one of which is by simply creating shell scripts that you can run, which call the CLI.
...
# Generate documentation with Qivryn CLI
echo "Running Qivryn agent to generate documentation..."
qivryn -config "$QIVRYN_ORG/$QIVRYN_CONFIG" \
   --auto \
   --allow Write \
   -p \
   --prompt ./context.txt \
...
Ready to implement automated documentation with Qivryn CLI? Here are some helpful resources to get you started:

<CardGroup cols={2}>
  <Card title="Qivryn CLI Guide" icon="terminal" href="/guides/cli">
    Learn the fundamentals of using Qivryn CLI for automated coding tasks and headless workflows.
  </Card>
  ...
  <Card title="Video: Leverage AI to help with your docs" icon="video" href="https://www.youtube.com/watch?v=rJ2taa8OLvY">
    Checkout this video from Tetrate about using Qivryn Agents to help with writing your docs.
  </Card>
