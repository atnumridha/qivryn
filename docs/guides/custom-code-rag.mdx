...
3. Use a recursive, abstract syntax tree (AST)-based strategy: this is the most exact, but most complex. In most cases you can achieve high quality results by using (1) or (2), but if you'd like to try this you can find a reference example in [our code chunker](https://github.com/atnumridha/qivryn/blob/main/core/indexing/chunk/code.ts) or in [LlamaIndex](https://docs.llamaindex.ai/en/stable/api_reference/node_parsers/code/).
...
To integrate your custom RAG system with Qivryn, you'll create an MCP (Model Context Protocol) server. MCP provides a standardized way for AI tools to access external resources.
...
### Configure Qivryn to use your MCP server

Add your MCP server to Qivryn's configuration:
