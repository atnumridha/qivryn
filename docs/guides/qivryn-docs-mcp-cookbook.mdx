---
title: "Contributing to Qivryn with Model Context Protocol (MCP)"
description: "Use the Qivryn Docs MCP to write cookbooks, guides, and documentation with AI-powered workflows."
sidebarTitle: "Qivryn Docs MCP Cookbook"
---

<Card title="What You'll Build" icon="book-open-cover">
  Master using the Qivryn Docs MCP to contribute documentation, create cookbooks, and maintain consistency across Qivryn's docs - all through natural language prompts.
</Card>
...
- Read: [Contributing to Qivryn Documentation](/CONTRIBUTING) for setup instructions
- Forked the [atnumridha/qivryn](https://github.com/atnumridha/qivryn) repository
- Node.js 20+ and Qivryn CLI installed

<Warning>
  This cookbook assumes you've read the setup in the [CONTRIBUTING guide](/CONTRIBUTING). If you haven't, start there first.
</Warning>

## Qivryn Docs MCP Setup

<Card title="Fastest Path to Success" icon="rocket">
  Use the pre-built [Docs Assistant - Mintlify agent](https://qivryn.ai/qivryn/docs-mintlify) that includes the Qivryn Docs MCP and is ready to use immediately.
</Card>

<Tabs>
  <Tab title="⚡ Quick Start (Recommended)">
    ```bash
    # From your Qivryn docs directory
    qivryn --config qivryn/docs-mintlify
    ```

    This agent includes:
    - **Qivryn Docs MCP** for searching Qivryn documentation
  ...
  <Tab title="🛠️ Custom Agent">
    ...
    1. [Qivryn Docs MCP](https://qivryn.ai/qivryn/qivryn-docs-mcp)
    2. [Mintlify Technical Writing Rule](https://qivryn.ai/mintlify/technical-writing-rule)
...
## What is the Qivryn Docs MCP?

<Card title="Qivryn Docs MCP" icon="circle-info">
  A Model Context Protocol server built with [Mintlify's MCP generation](https://www.mintlify.com/blog/generate-mcp-servers-for-your-docs) that enables semantic search across Qivryn documentation. [Learn more →](/reference/qivryn-mcp)
</Card>

The MCP helps you:
- **Find examples** from existing documentation
- **Maintain consistency** with established patterns
- **Source accurate information** about Qivryn features
...
Cookbooks show how to use Qivryn CLI with specific tools or services. Here's how to create one using the Qivryn Docs MCP:
...
<Steps>
  <Step title="Research Existing Cookbooks">
    ...
    **Prompt:**
    ```
    "Show me the structure of existing MCP cookbooks in the Qivryn docs.
  ...
  <Step title="Source Official Documentation">
    **Prompt:**
    ```
    "Using the Qivryn Docs MCP, find information about how GitHub MCP works.
    ...
    The agent will:
    - Search Qivryn docs for MCP patterns
  ...
  <Step title="Add to Navigation">
    **Prompt:**
    ```
    "Add my new github-mcp-qivryn-cookbook.mdx to the docs.json navigation
...
<Info>
  **Pro Tip:** The agent uses the Qivryn Docs MCP to maintain consistency with existing cookbooks automatically.
</Info>
...
<Steps>
  ...
  <Step title="Make Targeted Updates">
    **Prompt:**
    ...
    to include information about a new MCP server. Use the Qivryn
...
<Steps>
  <Step title="Research Similar Content">
    **Prompt:**
    ```
    "I want to create a guide for setting up Qivryn with Amazon Bedrock.
    Search the Qivryn docs for similar model provider setup guides and
  ...
  <Step title="Create the Guide">
    **Prompt:**
    ...
    Use the Qivryn Docs MCP to find accurate information about Qivryn's
...
**Prompt:**
```
"Create a cookbook for using Qivryn with PostgreSQL MCP. Follow the same
...
Use the Qivryn Docs MCP to find MCP configuration patterns and search the
...
**Prompt:**
```
"Create a cookbook showing how to use Qivryn to analyze Sentry errors.
...
**Prompt:**
```
"I want to create a cookbook for using Qivryn to work with OpenAPI specs.

Show how to:
1. Load OpenAPI specs into Qivryn's context
2. Generate API client code from specs
3. Create tests based on API endpoints
4. Update documentation when APIs change

Use the Qivryn Docs MCP to find how context providers work, then combine
...
<CardGroup cols={2}>
  <Card title="Multi-Source Research" icon="magnifying-glass">
    ```bash
    "Use the Qivryn Docs MCP to understand how agents
    work in Qivryn, then search the web for Anthropic's
    Computer Use MCP. Create a cookbook showing how to
    combine Qivryn agents with Computer Use for
    automated testing."
    ```
  </Card>
  ...
  <Card title="Cross-Reference Documentation" icon="link">
    ```bash
    "Find all mentions of MCP servers across Qivryn
  ...
  <Card title="Example Extraction" icon="code">
    ...
    configuration from the Qivryn docs. Format them
...
Add automated documentation checks to your PR workflow using the Qivryn Docs MCP agent:
...
jobs:
  check-docs:
    ...
    steps:
      ...
      - name: Install Qivryn CLI
        run: npm i -g @qivryn/cli

      - name: Analyze Changes for Documentation Needs
        id: analyze
        env:
          QIVRYN_API_KEY: ${{ secrets.QIVRYN_API_KEY }}
        run: |
          ...
          # Use Qivryn agent to check if docs need updates
          ...
          qivryn --config qivryn/docs-mintlify -p "$PROMPT" --auto > analysis.md || {
...
- [Qivryn Docs MCP Cookbook](/guides/qivryn-docs-mcp-cookbook)

*This analysis was generated using the Qivryn Docs MCP agent.*
EOF
...
<Info>
  This workflow uses the Qivryn Docs MCP agent to analyze code changes and automatically comment on PRs when documentation updates are recommended.
</Info>
...
<Steps>
  ...
  <Step title="Share Your MCP">
    Share your docs MCP so others can use it with Qivryn agents.
  </Step>
</Steps>

<Info>
  The [Qivryn Docs MCP](/reference/qivryn-mcp) itself was built this way!
</Info>
...
| Task | Prompt |
|:-----|:-------|
| **Find structure** | "Show me the structure of existing cookbooks in Qivryn docs" |
| **Create cookbook** | "Create a cookbook for [tool] MCP following the dlt cookbook structure" |
| **Update docs** | "Update [file] to include information about [feature], using Qivryn Docs MCP to find examples" |
| **Add navigation** | "Add [file] to docs.json under the [section] section" |
| **Check consistency** | "Review this file for consistency with other Qivryn documentation" |
| **Fix formatting** | "Review for Mintlify formatting issues and fix any problems" |
| **Extract examples** | "Find all examples of [topic] in Qivryn docs" |
| **Research feature** | "Use Qivryn Docs MCP to explain how [feature] works in Qivryn" |

---

## Resources

### Qivryn Documentation
- [Contributing Guide](/CONTRIBUTING) - Setup and submission process
- [Qivryn Docs MCP Reference](/reference/qivryn-mcp) - MCP server details
...
<CardGroup cols={2}>
  ...
  <Card title="Use Docs Agent" icon="robot" href="https://qivryn.ai/qivryn/docs-mintlify">
