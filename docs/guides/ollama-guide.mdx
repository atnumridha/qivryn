---
title: "Using Ollama with Qivryn: A Developer's Guide"
description: "Complete guide to setting up Ollama with Qivryn for local AI development. Learn installation, configuration, model selection, performance optimization, and troubleshooting for privacy-focused offline coding assistance"
---
...
## How to Configure Ollama with Qivryn

There are multiple ways to configure Ollama models in Qivryn:
...
```yaml title="~/.qivryn/configs/config.yaml"
...
<Warning>
  **Important**: Blocks only provide configuration - you still need to pull
  the model locally. The block `ollama/deepseek-r1-32b` configures Qivryn
  to use `model: deepseek-r1:32b`, but the actual model must be installed:
  ```bash
  # Check what the block expects (view on qivryn.ai)
...
Qivryn can automatically detect available Ollama models. You can configure this in your YAML:

```yaml title="~/.qivryn/config.yaml"
models:
  - name: Autodetect
    provider: ollama
    model: AUTODETECT
...
3. Qivryn will scan for available Ollama models
...
<Note>
  ...
  available models. When set to `AUTODETECT`, Qivryn will dynamically populate
...
Qivryn may use a higher default context length than other tools. Reduce `contextLength` in your config (e.g., to 2048), or try a smaller model. See [Ollama provider troubleshooting](/customize/model-providers/top-level/ollama#troubleshooting) for details.
...
async def create_user(user: User):
    # Qivryn will help complete this implementation
...
Ollama with Qivryn provides a powerful local development environment for AI-assisted coding. You now have complete control over your AI models, ensuring privacy and enabling offline development workflows.

---

_This guide is based on Ollama v0.11.x and Qivryn v1.1.x. Please check for updates regularly._
