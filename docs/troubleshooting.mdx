...
description: "Comprehensive guide to resolving common issues with Qivryn, including logging, keyboard shortcuts, networking problems, model capabilities, and extension configuration troubleshooting"
---
...
#### Prompt Logs (Qivryn Console)

To view prompt logs/analytics, you can enable the Qivryn Console.

1. Open VS Code settings (`cmd/ctrl` + `,`)
2. Search for the setting "Qivryn: Enable Console" and enable it
3. Reload the window
4. Open the Qivryn Console by using the command palette (`cmd/ctrl` + `shift` + `P`) and searching for "Qivryn: Focus on Qivryn Console View"

![Qivryn Console](/images/images/qivryn-console-d387a10c2918c117c6c253a3b5f18c22.png)

### JetBrains

Open `~/.qivryn/logs/core.log` to view the logs for the Qivryn plugin. The most recent logs are found at the bottom of the file.
...
We are constantly making fixes and improvements to Qivryn, but the latest changes remain in a "pre-release" version for roughly a week so that we can test their stability. If you are experiencing issues, you can try the pre-release by going to the Qivryn extension page in VS Code and selecting "Switch to Pre-Release" as shown below.
...
For VS Code, All versions are hosted on the Open VSX Registry [here](https://open-vsx.org/extension/Qivryn/qivryn). Once you've downloaded the extension, which will be a .vsix file, you can install it manually by following the instructions [here](https://code.visualstudio.com/docs/editor/extension-gallery#_install-from-a-vsix).

You can find older versions of the JetBrains extension on their [marketplace](https://plugins.jetbrains.com/plugin/22707-qivryn), which will walk you through installing from disk.

## Keyboard shortcuts not resolving

If your keyboard shortcuts are not resolving, you may have other commands that are taking precedence over the Qivryn shortcuts. You can see if this is the case, and change your shortcut mappings, in the configuration of your IDE.
...
This issue typically affects macOS users with large development environments and is being tracked in [#7870](https://github.com/atnumridha/qivryn/issues/7870) and [#6699](https://github.com/atnumridha/qivryn/issues/6699).

## SSL certificate errors

If Qivryn can reach your model endpoint but cannot verify its TLS certificate chain, you may see `fetch failed` alongside errors such as:
...
1. Reproduce the error in the Qivryn logs so you can confirm the exact hostname that failed.
...
5. Verify the fix with curl before retrying in Qivryn:
...
You can create a GitHub issue [here](https://github.com/atnumridha/qivryn/issues/new?assignees=&labels=bug&projects=&template=bug-report-%F0%9F%90%9B.md&title=), providing details of your problem, and we'll be able to help you out more quickly.
