...
qivryn --allow "Write(**/*.ts)"

# Allow bash but not for install commands
qivryn --allow Bash --exclude "Bash(npm install*)"
```

## `permissions.yaml`

Persistent permissions are stored in `~/.qivryn/permissions.yaml`. This file is updated when you choose "Qivryn + don't ask again" in the TUI approval prompt.

```yaml
# ~/.qivryn/permissions.yaml
allow:
  - Read(*)
  - Write(**/*.ts)
...
```bash
qivryn --auto       # Allow all tools without prompting
qivryn --readonly   # Plan mode — read-only tools only, no file writes
```
