...
Run `qivryn` to start an interactive session. You get a prompt where you can type messages, reference files with `@`, and use slash commands. `qivryn` uses the same underlying agent as the Qivryn IDE extensions.
...
When you're connected to a remote environment, Qivryn also adds remote-only slash commands such as `/diff` and `/apply`.
...
- **Qivryn** — approve this call
- **Qivryn + don't ask again** — approve and save a policy rule to `~/.qivryn/permissions.yaml`
