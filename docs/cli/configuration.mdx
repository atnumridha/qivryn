...
`qivryn` resolves its configuration (models, MCP servers, rules, etc.) from several sources, in this order:

1. **`--config` flag** — a file path passed at launch
2. **Saved config** — the last-used configuration, persisted across sessions
3. **Default** — `~/.qivryn/config.yaml`

## `--config` flag

Point `qivryn` at a local YAML file:

```bash
qivryn --config ./my-config.yaml
```
...
`qivryn` looks for `~/.qivryn/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
...
qivryn --rule ./rules/style.md
qivryn --rule "Always use TypeScript strict mode"

# Load an agent file
qivryn --agent my-org/pr-reviewer
```
