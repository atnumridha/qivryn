...
import com.github.qivryn.qivrynintellijextension.utils.getAltKeyLabel
...
fun getDiffDirectory(): File {
    ...
    val diffDirPath = Paths.get(homeDirectory).resolve(".qivryn").resolve(".diffs").toString()
...
class DiffManager(private val project: Project) : DumbAware {
    ...
    fun acceptDiff(file2: String?) {
        ...
        project.service<QivrynPluginService>().ideProtocolClient?.sendAcceptRejectDiff(true, diffInfo.stepIndex)
    ...
    fun rejectDiff(file2: String?) {
        ...
        qivrynPluginService.ideProtocolClient?.deleteAtIndex(diffInfo.stepIndex)
        qivrynPluginService.ideProtocolClient?.sendAcceptRejectDiff(false, diffInfo.stepIndex)
        qivrynPluginService.coreMessenger?.request("cancelApply", null, null) {}
        cleanUpFile(file)
    }
    ...
    private fun openDiffWindow(
        ...
        val diffRequest = SimpleDiffRequest("Qivryn Diff", content1, content2, "Old", "New")
