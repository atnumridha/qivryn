...
class QivrynBrowser(
    ...
    private val log = Logger.getInstance(QivrynBrowser::class.java.simpleName)
    ...
    init {
        CefApp.getInstance().registerSchemeHandlerFactory("http", "qivryn", CustomSchemeHandlerFactory())
        ...
        myJSQueryOpenInBrowser.addHandler { msg: String? ->
            ...
            if (MessageTypes.PASS_THROUGH_TO_CORE.contains(messageType)) {
                project.service<QivrynPluginService>().coreMessenger?.request(messageType, data, messageId) { data ->
            ...
            if (msg != null) {
                project.service<QivrynPluginService>().ideProtocolClient?.handleMessage(msg) { data ->
                    sendToWebview(
                        messageType,
                        mapOf(
                            "status" to "success",
        ...
        project.service<QivrynPluginService>().onProtocolClientInitialized {
    ...
    internal companion object {
        ...
        private fun getGuiUrl() =
            System.getenv("GUI_URL") ?: "http://qivryn/index.html"
