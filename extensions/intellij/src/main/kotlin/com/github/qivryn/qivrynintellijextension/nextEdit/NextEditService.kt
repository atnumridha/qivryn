package com.github.qivryn.qivrynintellijextension.nextEdit
...
class QivrynNextEditService(private val project: Project) : NextEditService {
    private val displayedCompletions = mutableMapOf<String, NextEditOutcome>()

    private val coreMessenger: CoreMessenger?
        get() = project.service<QivrynPluginService>().coreMessenger
...
//                val nextEditItem = processedItem?.let { convertToNextEditItem(it) }
    ...
    private fun parseProcessedItem(response: Any?): ProcessedItem? {
        return try {
            ...
            val range = com.github.qivryn.qivrynintellijextension.Range(startPosition, endPosition)
            val location = com.github.qivryn.qivrynintellijextension.RangeInFile(filepath, range)
