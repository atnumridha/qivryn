...
describe("asciiArt", () => {
  ...
  describe("getDisplayableAsciiArt", () => {
    it("should return full ASCII art when terminal is wide enough", () => {
      ...
      expect(result).toBe(QIVRYN_ASCII_ART);
    });

    it("should return QV ASCII art version when terminal is too narrow", () => {
      ...
      expect(result.length).toBeLessThan(QIVRYN_ASCII_ART.length / 2);
    });

    it("should return QV ASCII art version when terminal is below threshold", () => {
