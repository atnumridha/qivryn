  ...
  if (reviews.length === 0) {
    ...
    console.log(
      chalk.yellow("\nNo reviews found. To add reviews:\n") +
        chalk.dim(
          "  1. Create .qivryn/agents/my-review.md with agent instructions\n",
        ) +
        chalk.dim(
          "  2. Or specify an agent: qivryn review --review-agents org/agent-name\n",
        ) +
        chalk.dim("  3. Or configure reviews on https://qivryn.ai\n"),
    );
    process.exit(0);
  }
