...
4. Permissions in `~/.qivryn/permissions.yaml`
5. Default policies
...
qivryn --allow Read --ask Write --exclude Bash

# Start in plan mode (read-only tools only)
qivryn --readonly "Help me understand this codebase"

# Use mode switching during chat
qivryn "Let me work on this feature"  # Starts in normal mode
...
## `~/.qivryn/permissions.yaml` (personal settings)
...
```yaml title="~/.qivryn/permissions.yaml"
allow:
  - Read(*)
...
qivryn -p --allow write_file "Write a hello world script"

# Headless mode with wildcard permission (allow all tools)
qivryn -p --allow "*" "Write and run a script"

# Headless mode with specific restrictions
qivryn -p --exclude run_terminal_command "Clean up the codebase"
```
