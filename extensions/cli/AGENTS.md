...
This is a CLI tool for Qivryn AI that provides an interactive AI-assisted development experience. The architecture consists of:
...
3. **Qivryn SDK Integration** (`src/qivrynSDK.ts`): Initializes the Qivryn SDK client with:
...
7. **Hooks System** (`src/hooks/`): Event interception system for extending CLI behavior
   ...
   - **Config locations** (lowest to highest precedence):
     - `~/.claude/settings.json`, `~/.qivryn/settings.json` (user-global)
     - `.claude/settings.json`, `.qivryn/settings.json` (project)
     - `.claude/settings.local.json`, `.qivryn/settings.local.json` (project-local)
