...
The Qivryn CLI uses esbuild to bundle the application along with local packages (`@qivryn/config-yaml` and `@qivryn/openai-adapters`) into a single distributable file. This ensures that users who install the CLI from npm don't need to worry about local file references.
...
- Include local packages (`@qivryn/config-yaml`, `@qivryn/openai-adapters`) directly in the bundle
- Keep problematic or native dependencies external (e.g., `winston`, `express`)
- Create a wrapper script (`dist/qivryn.js`) with the proper shebang for CLI execution
...
2. Users install via `npm install -g @qivryn/cli`
3. The `qivryn` command becomes available globally
