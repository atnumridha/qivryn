...
export class VerticalDiffManager {
  ...
  // Listener for user doc changes is disabled during updates to the text document by Qivryn
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while Qivryn makes changes
    this.disableDocumentChangeListener();
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand("setContext", "qivryn.diffVisible", true);
    ...
    vscode.commands.executeCommand("setContext", "qivryn.streamingDiff", true);
  ...
  async streamEdit({
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by Qivryn
      // Before adding this back, we need to distinguish between human and Qivryn
    ...
    try {
      ...
      if (abortController.signal.aborted) {
        void vscode.commands.executeCommand("qivryn.rejectDiff");
      }
