      ...
      .filter(
        (s) =>
          ...
          (s.filepath !== currentFilepath &&
            // Exclude Qivryn's own output as it makes it super-hard for users to test the autocomplete feature
            // while looking at the prompts in the Qivryn's output
            !s.filepath.startsWith("output:extension-output-Qivryn.qivryn")),
      )
      .sort((a, b) => b.timestamp - a.timestamp)
