...
export class GUIActions {
  public static moveQivrynToSidebar = async (driver: WebDriver) => {
    ...
    await GUIActions.executeFocusQivrynInputShortcut(driver);
  ...
  public static switchToReactIframe = async () => {
    ...
    let qivrynIFrame: WebElement | undefined = undefined;
    for (let i = 0; i < iframes.length; i++) {
      const iframe = iframes[i];
      const src = await iframe.getAttribute("src");
      if (src.includes("extensionId=Qivryn.qivryn")) {
        qivrynIFrame = iframe;
        break;
      }
    }
    ...
    if (!qivrynIFrame) {
      throw new Error("Could not find Qivryn iframe");
    }

    await driver.switchTo().frame(qivrynIFrame);
  ...
  public static toggleGui = async () => {
    return TestUtils.waitForSuccess(() =>
      new Workbench().executeCommand("qivryn.focusQivrynInput"),
    );
  };
  ...
  public static async executeFocusQivrynInputShortcut(driver: WebDriver) {
    return driver
      .actions()
