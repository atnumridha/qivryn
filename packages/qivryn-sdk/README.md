...
This SDK provides programmatic access to Qivryn's Hub APIs and functionality.

## Overview

The Qivryn SDK is structured into separate language-specific packages, currently with implementations for TypeScript and Python, with more languages planned for the future:
...
1. OpenAPI-generated clients for the Qivryn Hub API
2. A wrapper layer that exposes a `Qivryn.from()` method to easily initialize and use Qivryn assistants with an OpenAI-compatible interface

## For End Users

End users should install the published packages directly:

- For TypeScript/JavaScript: `npm install @qivryn/sdk`
- For Python: `pip install qivryn`
