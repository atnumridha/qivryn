...
import { Qivryn } from "../src/Qivryn.js";

dotenv.config();

/**
 * This test is `.skip`'d for now becasue it requires a real API key
 * and a real assistant to be set up in the Qivryn Hub.
 *
 * The primary use case for now is to aid in local iteration on the SDK.
 */
test.skip("should make a real API call to the Qivryn service", async () => {
  const apiKey = process.env.QIVRYN_API_KEY;

  if (!apiKey) {
    throw new Error("QIVRYN_API_KEY not found in environment variables");
  }
