import { ConfigJson } from "@qivryn/config-types";
...
function convertContext(configJson: ConfigJson): ContextYaml[] {
  const context: ContextYaml[] =
    configJson.contextProviders?.map((ctx) => {
      ...
      return {
        uses: `qivryn/${ctx.name === "open" ? "open-files" : ctx.name}-context`,
        with: ctx.params,
      };
    }) ?? [];
...
function convertCustomCommand(
  ...
  return {
    ...
    prompt: (cmd as any).prompt, // The type is wrong in @qivryn/config-types
  };
}
