...
/**
 * Convert from JSON schema (used in Claude Desktop) to YAML schema (used in Qivryn)
 */
export function convertJsonMcpConfigToYamlMcpConfig(
  ...
  if ("command" in jsonConfig) {
    if (jsonConfig.envFile) {
      warnings.push(
        `envFile is not supported in Qivryn MCP configuration (server "${name}"). Environment variables from file will not be used.`,
      );
    }
...
/**
 * Convert from YAML schema (used in Qivryn) to JSON schema (e.g. used in Claude Desktop)
 */
