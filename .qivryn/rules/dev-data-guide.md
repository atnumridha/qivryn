...
# Qivryn AIelopment Data (Dev Data) Guide
...
- **Default storage**: `~/.qivryn/dev_data/`
- **Event files**: `~/.qivryn/dev_data/{version}/{eventName}.jsonl`
...
Dev data is configured through `data` blocks in your Qivryn config:

```yaml
data:
  - name: "Local Development Data"
    destination: "file:///Users/developer/.qivryn/dev_data"
    schema: "0.2.0"
    level: "all"
...
1. **Check local storage**: Verify files are being created in `~/.qivryn/dev_data/`
2. **Validate schemas**: Ensure event data matches expected schema format
3. **Review configuration**: Check `data` blocks in Qivryn config
...
This guide provides the foundation for understanding and working with Qivryn's dev data system. Always prioritize user privacy and follow established patterns when making changes.
