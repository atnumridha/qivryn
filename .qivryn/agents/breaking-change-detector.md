...
1. **CLI command renames or removals** - If a command registered in `extensions/cli/src/commands/` is renamed, removed, or has its flags changed, check that:
   ...
   - Agent definitions in `.qivryn/agents/` don't reference the old command
...
4. **URL changes** - If any hardcoded URLs (e.g., `hub.qivryn.ai`, `api.qivryn.ai`) are changed, scan for stale references across the repo.
