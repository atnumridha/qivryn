...
const siteIndexingConfigsAreEqual = (
  siteConfig1: SiteIndexingConfig,
  siteConfig2: SiteIndexingConfig,
  contConfig1: QivrynConfig | undefined,
  contConfig2: QivrynConfig,
) => {
  return (
...
export default class DocsService {
  ...
  async indexAndAdd(
    ...
    try {
      await provider.embed(["qivryn-test-run"]);
    } catch (e) {
  ...
  private async syncDocs(
    oldConfig: QivrynConfig | undefined,
    newConfig: QivrynConfig,
    ...
    try {
      ...
      for (const doc of newConfigDocs) {
        ...
        if (currentIndexedDoc) {
          ...
          // TODO: Changes to the docs config made while Qivryn isn't running won't be caught
          if (
            oldConfigDoc &&
            ...
            if (forceReindex) {
              ...
              // This is a temperary fix to catch the changes to the docs config that were made when Qivryn isn't running
