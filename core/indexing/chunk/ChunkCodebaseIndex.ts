...
import { IQivrynServerClient } from "../../qivrynServer/interface.js";
...
export class ChunkCodebaseIndex implements CodebaseIndex {
  ...
  constructor(
    ...
    private readonly qivrynServerClient: IQivrynServerClient,
  ...
  async *update(
    ...
    if (this.qivrynServerClient.connected) {
      try {
        const keys = results.compute.map(({ cacheKey }) => cacheKey);
        const resp = await this.qivrynServerClient.getFromIndexCache(
          keys,
          "chunks",
          repoName,
        );
