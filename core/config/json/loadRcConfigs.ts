...
import { QivrynRcJson, FileType, IDE } from "../..";
import { joinPathsToUri } from "../../util/uri";

export async function getWorkspaceRcConfigs(ide: IDE): Promise<QivrynRcJson[]> {
  try {
        ...
        const rcFiles = ls
          .filter(
            (entry) =>
              ...
              entry[0].endsWith(".qivrynrc.json"),
          )
    ...
    return rcFiles
      .flat()
      .map((file) => JSONC.parse(file) as unknown as QivrynRcJson);
  } catch (e) {
