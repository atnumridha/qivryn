import { FQSN, SecretResult, SecretType } from "@qivryn/config-yaml";
import {
  afterEach,
  beforeEach,
  describe,
...
describe("LocalPlatformClient", () => {
  const testFQSN: FQSN = {
    ...
    secretName: "TEST_QIVRYN_SECRET_KEY",
  };
  ...
  beforeEach(
    ...
    () => {
      ...
      envKeyValues = {
        TEST_QIVRYN_SECRET_KEY: secretValue,
  ...
  describe("searches for secrets in local .env files", () => {
    let getQivrynDotEnv: Mock;
    beforeEach(async () => {
      const utilPaths = await import("../../util/paths");
      getQivrynDotEnv = vi.fn(() => envKeyValues);
      utilPaths.getQivrynDotEnv = getQivrynDotEnv;
    });

    test("should be able to get secrets from ~/.qivryn/.env files", async () => {
      const localPlatformClient = new LocalPlatformClient(testIde);
      const resolvedFQSNs = await localPlatformClient.resolveFQSNs([testFQSN]);
      expect(getQivrynDotEnv).toHaveBeenCalled();
  ...
  describe("should be able to get secrets from workspace .env files", () => {
    test("should get secrets from <workspace>/.qivryn/.env and <workspace>/.env", async () => {
      ...
      const randomValueForQivrynDirDotEnv =
        "qivryn-dir-" + Math.floor(Math.random() * 100);
      const randomValueForWorkspaceDotEnv =
        "dotenv-" + Math.floor(Math.random() * 100);

      testIde.readFile = vi.fn(async (fileUri: string) => {
        // fileUri should contain .qivryn/.env and not .env
        if (fileUri.match(/.*\.qivryn\/\.env.*/gi)?.length) {
          return (
            envKeyValuesString.split("\n")[0] + randomValueForQivrynDirDotEnv
          );
        }
        // filUri should contain .env and not .qivryn/.env
        else if (fileUri.match(/.*(?<!\.qivryn\/)\.env.*/gi)?.length) {
          return (
    ...
    test("should first get secrets from <workspace>/.qivryn/.env and then <workspace>/.env", async () => {
      ...
      // we check that workspace <workspace>.qivryn/.env does not override the <workspace>/.env secret
      expect(
        (resolvedFQSNs[0] as SecretResult & { value: unknown })?.value,
      ).toContain(randomValueForQivrynDirDotEnv);
      expect(
        (resolvedFQSNs[0] as SecretResult & { value: unknown })?.value,
  ...
  describe("should be able to get secrets from process.env", () => {
    ...
    beforeEach(async () => {
      ...
      const utilPaths = await import("../../util/paths");
      utilPaths.getQivrynDotEnv = vi.fn(() => ({}));
    ...
    test("should prioritize local ~/.qivryn/.env file over process.env", async () => {
      const localEnvFileValue = "secret-from-local-dot-qivryn-env";
      const utilPaths = await import("../../util/paths");
      utilPaths.getQivrynDotEnv = vi.fn(() => ({
    ...
    test("should prioritize workspace .env files over process.env", async () => {
      const workspaceQivrynEnvValue = "secret-from-workspace-qivryn-env";
      testIde.fileExists = vi.fn(async (fileUri: string) =>
        // Only mock existence for <workspace>/.qivryn/.env
        fileUri.includes(".qivryn/.env"),
      );
      testIde.readFile = vi.fn(async (fileUri: string) => {
        if (fileUri.includes(".qivryn/.env")) {
          return `${testFQSN.secretName}=${workspaceQivrynEnvValue}`;
        }
        return "";
      });
