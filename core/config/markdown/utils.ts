import { RULE_FILE_EXTENSION, sanitizeRuleName } from "@qivryn/config-yaml";
import { joinPathsToUri } from "../../util/uri";

function createRelativeRuleFilePathParts(ruleName: string): string[] {
  const safeRuleName = sanitizeRuleName(ruleName);
  return [".qivryn", "rules", `${safeRuleName}.${RULE_FILE_EXTENSION}`];
}
...
/**
 * Creates the file path for a rule in the workspace .qivryn/rules directory
 */
