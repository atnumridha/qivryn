...
class MCPContextProvider extends BaseContextProvider {
  ...
  /**
   * Qivryn experimentally supports resource templates (https://modelcontextprotocol.io/docs/concepts/resources#resource-templates)
    ...
    return await Promise.all(
      contents.map(async (resource) => {
        if (!("text" in resource) || typeof resource.text !== "string") {
          throw new Error(
            "Qivryn currently only supports text resources from MCP",
          );
        }
        return {
          name: resource.uri,
